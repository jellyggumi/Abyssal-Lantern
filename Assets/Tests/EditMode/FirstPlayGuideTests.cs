...
namespace CastleBusters.Tests
{
    ...
    public sealed class FirstPlayGuideTests
    {
        private static FirstPlayGuide.Observation Frame(
            ...
            bool gameOver = false)
        {
            return new FirstPlayGuide.Observation(acknowledged, playerTurn, aiming, resolving, gameOver);
        }
        ...
        public void HappyPath_WalksEveryStepInOrder_OnTheActionEachStepTeaches()
        {
            ...
            // Release advances only when the shot is committed (volley resolving).
            ...
            // The reply beat completes on the player's NEXT turn, which requires having
            // actually seen the enemy's turn happen.
            Assert.That(guide.Advance(Frame(playerTurn: true)), Is.False,
                "the player's own turn winding down is not yet the enemy's reply");
            Assert.That(guide.Advance(Frame(playerTurn: false)), Is.False,
                "the enemy acting is observed, not completed — the step ends on control returning");
            Assert.That(guide.Advance(Frame(playerTurn: true)), Is.True);
            ...
            Assert.That(guide.Advance(Frame(acknowledged: true, aiming: true, resolving: true)), Is.False,
        ...
        public void WeakRelease_FallsBackToDraw_InsteadOfNarratingALaunchThatNeverHappened()
        {
            ...
            guide.Advance(Frame(aiming: true));             // Draw -> Release

            // The launcher refuses a too-shallow pull: aiming ends with no resolve.
            Assert.That(guide.Advance(Frame(aiming: false, resolving: false)), Is.True);
            Assert.That(guide.Current, Is.EqualTo(FirstPlayGuide.Step.Draw),
                "a refused shot must return to the draw instruction, not stay on release");

            // And the retry still works.
            guide.Advance(Frame(aiming: true));
            Assert.That(guide.Current, Is.EqualTo(FirstPlayGuide.Step.Release));
        }
        ...
        public void KeyboardCommit_SkipsTheDragObservation_ButNeverTheReplyBeat()
        {
            ...
            Assert.That(guide.Advance(Frame(resolving: true, playerTurn: true)), Is.True);
            Assert.That(guide.Current, Is.EqualTo(FirstPlayGuide.Step.EnemyReply),
        ...
        public void EnemyResolve_DoesNotAdvanceTheDrawStep()
        {
            ...
            Assert.That(guide.Advance(Frame(resolving: true, playerTurn: false)), Is.False);
            Assert.That(guide.Current, Is.EqualTo(FirstPlayGuide.Step.Draw));
        }
        ...
        public void ClockHold_CoversExactlyThePreShotSteps()
        {
            ...
            guide.Advance(Frame(resolving: true));
            ...
            guide.Advance(Frame(playerTurn: false));
            guide.Advance(Frame(playerTurn: true));
