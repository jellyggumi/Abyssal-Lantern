...
- **First-volley tempo compensation**: damage ownership and the 0.5 scalar are captured when the turn-0 player action is committed or its deploy is created, then applied at the eventual damage event. Production PlayMode covers committed melee, arrow impact, cannon splash, launched-barrel fuse, launched-unit → production field-keg handoff, and a same-frame competing hit after fatal destruction starts; delayed impacts and chained explosions preserve the capture after turn handoff (`TestResults/pr44-damage-hardened-v5.xml`, 13/13). Later and enemy-owned actions capture full damage. This is route-correctness only; G2 remains FAIL without a symmetric ≥20-match runtime win-rate sample.
